...
;; Version: $Id: analog.el,v 1.120 2005/08/16 18:15:25 mphodges-guest Exp $
...
(unless (string-equal user-full-name "Matt Hodges")
  (error "Development (unreleased) version of analog!"))
...
(defgroup analog nil
  ...
  :group 'tools
  :link '(url-link "http://mph-emacs-pkgs.alioth.debian.org/AnalogEl.html"))
...
(define-widget 'analog-lazy 'default
...
(define-widget 'analog-actions-widget 'analog-lazy
...
(define-widget 'analog-faces-widget 'analog-lazy
...
(define-widget 'analog-alerts-widget 'analog-lazy
           ...
           (repeat
            (list
             ...
             (cons :tag "Type of entry" (const type)
                   (choice :value ,analog-default-type
                           (const :tag "File" file)
             ...
             (cons :tag "Part of entry to show" (const position)
                   (choice :value ,analog-default-position
                           (const tail)
                           (const head)))
...
(defvar analog-last-group nil
  "The last group of entries viewed.")
  ...
  (let ((inhibit-read-only t)
    ...
    (when analog-alerts-warnings
      (let* ((warning-string (format "Analog Alert (%s)" (current-time-string)))
        ...
        (with-output-to-temp-buffer "*analog alerts*"
          (princ (concat dashes "\n" warning-string "\n" dashes "\n\n"
...
`analog-current-group'."
  ...
  (let ((list (copy-alist analog-group-list)))
    ...
    (setq analog-last-group analog-current-group
          analog-current-group
...
(defun analog-jump-to-last-group ()
  "Jump to the group last viewed."
  (interactive)
  (cond
   ((member analog-last-group analog-group-list)
    (let ((current-group analog-last-group)
          (last-group analog-current-group))
      (setq analog-current-group current-group
            analog-last-group last-group)
      (analog-update-entries-in-current-group)
      (analog-revert-buffer)))
   ((null analog-last-group)
    (message "No last group to jump to"))
   (t
    (message "Invalid last group: %s" analog-last-group)
    (setq analog-last-group nil))))
...
(defun analog-choose-entry ()
  ...
  (if analog-entries-in-current-group
      (let ((completion-ignore-case t)
            ...
            choice)
        (setq choice (car (assoc (completing-read "Choose entry: "
                                                  completions
                                                  nil t)
                                 completions)))
        (when choice
  ...
  (if analog-group-list
      (let ((completion-ignore-case t)
            (completions (mapcar (lambda (elt)
                                 ...
                                 analog-group-list))
            choice)
        (setq choice (car (assoc
                           (completing-read "Choose group: "
                                            completions
                                            nil t)
                           completions)))
        (when choice
          (setq analog-last-group analog-current-group
                analog-current-group choice)
          (analog-update-entries-in-current-group)
          (analog-revert-buffer)))
...
(defvar analog-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "j") 'analog-jump-to-last-group)
